...
//	}
	...
	public HashMap<String, String> getDetails() {
		...
		String result = this.prepareRestTransmission("Repository/" + Long.toString(id)).GetData();
	...
	public String storeRepository() {
		...
		try {
			...
			SingleStatementConnection stmtconn = (SingleStatementConnection) new DBAccessNG().getSingleStatementConnection();
			PreparedStatement statement = InsertIntoDB.Repository(stmtconn.connection, name, url, oaiUrl, Integer.parseInt(harvestAmount),
					Integer.parseInt(harvestPause), listRecords, Boolean.parseBoolean(testData), active);
	...
	public String delete() {
		...
		try {
			...
			SingleStatementConnection stmtconn = (SingleStatementConnection) new DBAccessNG().getSingleStatementConnection();
			PreparedStatement statement = DeleteFromDB.Repositories(stmtconn.connection, id);

			new DBHelper().save(stmtconn, statement);

			ctx.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "info.success_deleted", null));
	...
	public String deactivate() {
		...
		try {
			...
			SingleStatementConnection stmtconn = (SingleStatementConnection) new DBAccessNG().getSingleStatementConnection();
			PreparedStatement statement = UpdateInDB.Repository(stmtconn.connection, id, false);

			new DBHelper().save(stmtconn, statement);

			ctx.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "info.success_deactivated", null));
