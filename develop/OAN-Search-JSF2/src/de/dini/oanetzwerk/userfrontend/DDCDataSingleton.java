...
public final class DDCDataSingleton {
	...
	private void setParentBrowseBeanRecursive(DDCNaviNode node,
			BrowseBean parentBrowseBean) {
		...
		if (listNodes != null && !listNodes.isEmpty()) {
			for (DDCNaviNode subnode : listNodes)
				setParentBrowseBeanRecursive(subnode, parentBrowseBean);
		}
	}
	...
	public List<DDCNaviNode> getListDDCNaviNodes(BrowseBean parentBrowseBean) {
		...
		for (DDCNaviNode node : newListDDCNaviNodes)
			setParentBrowseBeanRecursive(node, parentBrowseBean);
	...
	private List<String[]> generateSimpleDDCCategorySums() {
		...
		if (rms.getStatus() != RestStatusEnum.OK) {
			...
			logger.error("received status " + rms.getStatus() + " ("
					+ rms.getStatusDescription() + ") for DDCCategories");
			return null;
		}
		...
		for (RestEntrySet res : rms.getListEntrySets()) {
			...
			while (it.hasNext()) {
				key = it.next();
				if ("DDC_Categorie".equals(key))
					strCat = res.getValue(key);
				if ("direct_count".equals(key))
					strDirectSum = res.getValue(key);
				if ("sub_count".equals(key))
					strSubSum = res.getValue(key);
				if ("name_deu".equals(key))
					strCatName = res.getValue(key);
			}
		...
		// Collections.sort(listDDCCategoriesAndSum, new
		// CategoryNumberComparator());
		Collections
				.sort(listDDCCategoriesAndSum, new CategoryCountComparator());

		return listDDCCategoriesAndSum;
	}

	private List<String[]> generateDirectDDCCategorySums(
			List<String[]> listSimpleSums) {
		...
		for (String entry[] : listSimpleSums) {
			...
			for (RestEntrySet res : rms.getListEntrySets()) {
				...
				while (it.hasNext()) {
					key = it.next();
					if (key.equals("sum"))
						listDDCCategoriesAndSum.add(new String[] { entry[0],
								res.getValue(key), wildcardCategory });
				}
		...
		for (int i = 0; i < 10; i++) {
			...
			for (int j = 1; j < 10; j++) {
				...
				for (int k = 1; k < 10; k++) {
					 ...
					 * DDCNaviNode node_lvl4 =
					 * getDDCNaviNodeForCat(strCategory_lvl4);
					 * 
					 * if(node_lvl4.getLongItemCount() > 0)
					 * node_lvl3.getListSubnodes().add(node_lvl4);
					 * 
					 * }
					 */

					if (node_lvl3.getLongItemCount() > 0)
						node_lvl2.getListSubnodes().add(node_lvl3);

				}

				if (node_lvl2.getLongItemCount() > 0)
					node_lvl1.getListSubnodes().add(node_lvl2);
			}

			if (node_lvl1.getLongItemCount() > 0)
				newList.add(node_lvl1);
		}
	...
	private List<DDCNaviNode> generateListDDCNaviNodes(List<String[]> flatList) {
		...
		/*
		 * for(String[] entry : flatList) { Pattern p = Pattern.compile(
		 * "[0-9]00"); Matcher m = p.matcher(entry[0]); if(m.matches()) {
		 * DDCNaviNode node = new DDCNaviNode(); node.setStrDDCValue(entry[0]);
		 * try { node.setLongItemCount(Long.parseLong(entry[1])); } catch
		 * (Exception ex) { node.setLongItemCount(-1); } String name =
		 * (String)mapDDCNames_de.get(entry[0]); node.setStrNameDE(name);
		 * nodeList.add(node); } }
		 */
		...
		for (int i = 0; i < 10; i++) {
			...
			for (int k = 1; k < 10; k++) {
				...
				if (node3.getLongItemCount() > 0)
					node1.getListSubnodes().add(node3);
			}
			...
			for (int j = 1; j < 10; j++) {
				...
				for (int k = 1; k < 10; k++) {
					...
					if (node3.getLongItemCount() > 0)
						node2.getListSubnodes().add(node3);
				}

				if (node2.getLongItemCount() > 0)
					node1.getListSubnodes().add(node2);

			}

			if (node1.getLongItemCount() > 0)
				nodeList.add(node1);
